...
conda create -c conda-forge --override-channels -name "$ENVNM" -y -f python=2.7 anaconda
...
conda install -y -c conda-forge matplotlib numpy scipy seaborn=0.8* scikit-learn=0.18* statsmodels=0.8.0 basemap requests 

printf "Installing packages from conda-forge...\n"
conda install --y -c conda-forge clusterpy=0.9.9 geopandas=0.2* palettable=3.* pysal=1.13* folium mplleaflet rasterio 
